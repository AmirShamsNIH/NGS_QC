...
def _build_execution_script(
    ...
    Every job's log and expected outputs from a previous run are deleted
    first, so the XLSX report never mistakes stale files for this run's.
    """
    clean_py = (
        "import glob, json, os, sys; m = json.load(open(sys.argv[1])); "
        "[os.remove(p) for j in m['jobs'] for pat in [j['log'], *j['outputs']] "
    ...
    return "\n".join([
        ...
        "",
        "# Step 0 – remove logs / outputs of a previous run of this study",
        f"python3 -c {shlex.quote(clean_py)} {shlex.quote(manifest_file)}",
        "",
